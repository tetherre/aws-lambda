...
```bash
mls-scraper$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mls-scraper$ sam local start-api
mls-scraper$ curl http://localhost:3000/
```
...
```bash
mls-scraper$ sam logs -n HelloWorldFunction --stack-name mls-scraper --tail
```
...
```bash
sam delete --stack-name mls-scraper
```
